...
float Hist::getCPU(int phyNodeID){
    PhyNode p = m_topo.get(phyNodeID);
    if(p.id == 0){
...
int Hist::getContainerSize(int funcType){
    return m_funcInfoMap.getSize(funcType);
}

//get the instantion cost of a function
float Hist::getInstanCost(int phyNodeID, int funcType){
...
float Hist::getRunCost(int phyNodeID, int funcType){
    float cpuFreq = getCPU(phyNodeID);
