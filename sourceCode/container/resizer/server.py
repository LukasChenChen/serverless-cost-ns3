...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        resize()
        self.end_headers()
        self.wfile.write(b'This is resizer app...')

    def do_POST(self):
        start = time.process_time()
        logging.warning('This is resizer app...')
        ...
        # logging.warning('print request', request)
        ...
        if request['Served'] == True:
            ...
            # logging.warning('Ingress ID: %s, "Deploynode: %s' % (request['Ingress']['ID'], request['DeployNode']['ID']))
            ...
            processing_delay = time.process_time() - start
            result = {
                "ID": request['ID'],
            ...
            # logging.warning('send latency result', result)
            response.write(str.encode(json.dumps(result)))
            self.wfile.write(response.getvalue())
        logging.warning('Finish response to resizer request...')
        logging.warning('Processing time: %f',time.process_time() - start)
