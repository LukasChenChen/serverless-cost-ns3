...
SERVER_HOST = '0.0.0.0'
SERVER_PORT = 3333
WEB_PAGE = "<html><head><title>This is Web Server</title></head></html>"
FILENAME = "test.html"
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        logging.warning('Processing time: %f seconds',time.time() - start)
    ...
    def do_POST(self):
        ...
        logging.warning('print request length %s', content_length)
        ...
        if request['Served'] == True:
            ...
            logging.warning('send latency result')
            response.write(str.encode(json.dumps(result)))
            self.wfile.write(response.getvalue())
        logging.warning('Finish response to post request...')
        logging.warning('Processing time: %f seconds',time.time() - start)
