...
func reconcileUnscheduledPods(interval float64, done chan struct{}, wg *sync.WaitGroup) {
	log.Println("start to reconcileUnscheduledPods")
	for {
		select {
...
func monitorUnscheduledPods(done chan struct{}, wg *sync.WaitGroup) {
	log.Println("monitorUnscheduledPods.....")
...
//TODO:this one control pod scheduling
func schedulePod(pod *Pod) error {
	log.Println("start to schedulePod.")
	nodes, err := fit(pod)
	if err != nil {
		return err
	}
	...
	node, err := selectNode(nodes, pod)
	log.Println("selectNode %s", node.Metadata.Labels["index"])
	if err != nil {
		return err
	}
	err = bind(pod, node)
...
func schedulePods() error {
	log.Println("start to schedulePods")
	processorLock.Lock()
